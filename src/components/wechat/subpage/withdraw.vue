        ...
        <div class="withdrawBank">
          <div style="display:flex">
            ...
            <div style="padding-top:.5rem;">
              ...
              <span class="withdrawtext">存管账户</span>
            </div>
          </div>
          <div>
          ...
          <div style="display:flex">
            <img style="width:4rem;height: 4rem;margin-right:.5rem" :src="list.bankURI" alt="">
            <div style="padding-top:.5rem;">
              <span style="font-size:1.4rem;
color: #202020;font-weight:bold;display:inline-block;margin-bottom:0.8rem">{{list.bankName}}</span><br>
              <span style="font-size: 1.2rem;
color: #202020;">尾号（{{list.cardLast}}）</span>
            </div>
    ...
    <!-- <div style="padding: .8rem 1.6rem;font-size: 1.4rem;line-height: normal;color: #202020;background: #F5F5F5;">换卡审核中</div> -->
  </div>
</template>
...
export default {
        ...
        _this.$http
            ...
            .then(function(res) {
                if (res.data.end == "ok") {
                    ...
                    _this.bankStatus = res.data.list[0].bankStatus;
                    _this.bankCardstatus = res.data.list[0].bankCardstatus;
    ...
    methods: {
        ...
        payShow() {
            ...
            //alert(formData);
            console.log(_this.rMoney);
            if (_this.rMoney < 0) {
                _this.$Toast("请输入大于1元的金额");
                return;
            } else if (_this.rMoney > 1000000) {
                _this.$Toast("单笔提现金额不能超过100万元");
                return;
            } else if (_this.rMoney > parseFloat(_this.leftMoney / 100)) {
                _this.$Toast("提现金额不能超过用户余额");
                return;
            } else if (_this.bankStatus == '1' ) {
              // 银行的状态 0:支持; 1:不支持
                _this.$Toast("通道维护中。暂不可提现");
                return;
            } else if (_this.bankCardstatus ==  '2'||this.bankCardstatus == '3') {
              // 银行卡的状态 0可用，2、3审核
                _this.$Toast("解绑流程未完成，暂不可操作");
                return;
            } else {
                _this.$http
