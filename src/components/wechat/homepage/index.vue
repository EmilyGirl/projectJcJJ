<template>
  <div id="index" class="index">
    <img @click="xiaoxiGG" class="indexInfo" src="./../../../assets/img/index/xiaoxi.png" alt="">
    <div class="headSwiper" v-if="swiperSlides.length>0">
      <swiper  :options="swiperOption">
        <swiper-slide v-for="(slide, index) in swiperSlides" :key="index" class='bannerSlide '>
          <a  :href="slide.hdUrl?slide.hdUrl:'javascript:void(0)'" >
    ...
    <!-- iconlist -->
    <!-- 公告轮播 -->
    <div class="content"  v-if="noticeTitle.length>0">
      <div class="announcement">
        <div class="textSwiper">
          ...
          <!--  v-if="noticeTitle.length>0" -->
          <swiper  :options="noticeOption">
            <swiper-slide v-for="(slide, index) in noticeTitle" :key="index" class='noticeSlide '>
              <!-- <a :href="slide.noticeUrl"> -->
              <p @click="gonggaoId(slide.noticeId)">{{slide.title?slide.title:' '}}</p>
              <!-- </a> -->
            </swiper-slide>
            <p @click="gonggaoId(noticeTitle[0].noticeId)">{{noticeTitle[0].title?noticeTitle[0].title:' '}}</p>
          </swiper>
        </div>
        <span @click="xiaoxiGG">更多</span>
      </div>
      <!-- 公告轮播 -->
      <!-- 注册 -->

      <!-- 判断为空 -->
      <div v-if="!!(userGuideMap&&userGuideMap.img)" class="register" @click="noLogin">
    ...
    <!-- 新手尊享 -->
    <div class="newHand" v-for="firstlist in freshProductList" @click="listFun(firstlist.id)">
        <div class="newhandTitle">
            <img style="width:2.6rem;margin-right:.6rem;padding-bottom: 0.9rem;" src="../../../assets/img/wechat/mine/xinshou.png" alt="">
            <span style="font-size:1.6rem;
color: #202020;font-weight:bold;">新手尊享</span>
            <span style="font-size:1.6rem;
color: #CECFD2;">&nbsp;&nbsp;|&nbsp;&nbsp;</span>
            <span style="font-size:1.6rem;
color: #202020;font-weight:bold;">{{ firstlist.title }}</span>
 <div class="profit1">
              <p>{{ firstlist.baseEarnings | fixNum }}%</p>
              <p style="font-size:1.5rem" v-if="firstlist.isJiangLi == 1" >+{{ parseFloat(firstlist.jiangLiEarnings) | fixNum }}%</p>
            </div>
            <div class="profit2">
              <p>借款期限{{ firstlist.lcqx }}天&nbsp;&nbsp;|&nbsp;&nbsp;{{firstlist.atleastMoney/100}}元起投</p>
            </div>
            <Button  :disabled='disabled'  :style="{backgroundColor: (parseInt(firstlist.leftCopies/100) > 0 ? '':'#C7CCD6'),'margin-top':'2rem!important'}"  >{{parseInt(firstlist.leftCopies/100) > 0 ? '授权出借':'已结束'}}</Button>
        </div>
    </div>
    <!-- 热门推荐 -->
    <div class="popularRecommend">
      ...
      <div v-if="firstProductList.length==0||firstProductList==' '">
        ...
        <div class="noviceMiddle">
          <div>
            <div class="profit">
              ...
              <p style="font-size:1.5rem" v-if="list.isJiangLi == 1">+{{ parseFloat(list.jiangLiEarnings) | fixNum(1) }}%</p>
            </div>
          ...
          <div>
           <div class="profit">
            ...
            <!-- <p style="padding-top:.6rem;font-size:1rem;color:#606060" v-if="parseFloat(list.leftCopies/100)<1000" >剩余{{ parseFloat(list.leftCopies/100).toFixed(2) }}元</p> -->
            <p style="padding-top:.6rem;font-size:1rem;color:#606060"  >剩余{{ parseFloat(list.leftCopies/1000000).toFixed(2) }}万元</p>
            </div>
          <div  :style="{ backgroundColor: (parseInt(list.leftCopies/100) > 0 ? '':'#C7CCD6')}" class="purchaseBtn">{{parseInt(list.leftCopies/100) > 0 ? '授权出借':'还款中'}}</div>
        </div>
        <div class="progressWrap">
          <div class="progress">
            <p>
    ...
    <!-- 累计统计 -->
    <div class="stat" style="margin-bottom:3rem">
  ...
  mounted: function() {
    ...
    this.$http
      ...
      .then(res => {
        ...
        this.noticeTitle = res.data.data.noticeTitle.slice(0,3);
        this.noticetitle=res.data.data.noticeTitle[0].title;
