...
import Button from '@/components/wechat/public/button/button.js'
Vue.use(Button);
...
// 三位 一个 ，
Vue.filter('toCurrencyString', function (money) {
	money =   money ||  0;
			return  parseInt(money).toFixed(2).replace(/\d(?=(?:\d{3})+\b)/g, "$&,"); // 这里看你是不是要小数
	 })
	//  \保留index 位小数  不四舍五入
	 Vue.filter('toFixNum', function (value, index) {
		  index = index || 2;
	      value = value || 0;
