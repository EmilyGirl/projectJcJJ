<template>
	<div class="activity">
		...
		<div><img src="../assets/img/d-1.png"/></div>
		<div><img src="../assets/img/d-2.png"/></div>
		<div><img src="../assets/img/d-3.png"/></div>
		<div><img src="../assets/img/d-4.png"/></div>
		<div class="double">
			<div class="double-name" v-for="(item,index) in items">
				<img :src="'../../static/img/dTOP0'+ index +'.png'"/>
				<span v-if="item.phone" class="ph">{{ item.phone }}</span>
				<span v-else class="ph">虚位以待</span>
				<span v-if="item.totalMoney"  class="mon">{{ item.totalMoney/100 }}元</span>
				<span v-else class="mon">-- &nbsp;--</span>

			</div>
			<div class="double-name"><img class="bang" src="../../static/img/TOP.png"/></div>
		</div>
		<div class="share-btn">
		...
		methods: {
			...
			loadData: function () {
				...
				_this.$http.post('/activity/getInvestmentRankings', {
						parameters: '{"activityId":"1","num":"10"}'
					})
					.then(function(res) {
						if(res.data.end == 'ok'){
