				...
				<span class="product-name">		<img    v-if="firstData1.qxType == 4"  style="width:2.4rem;padding-right:1rem;" src="./../../../assets/img/wechat/mine/xinshou.png" alt="">{{ title }}</span>
						...
						<div class="clearfix">
							<div class="middleLeft fl">
								<span>{{ baseEarnings | fixNum }}
									<span   v-if="isJiangLi  != 1">%</span>
									<var v-if="isJiangLi == 1">+{{ parseFloat(jiangLiEarnings) | fixNum(1) }}
										<span>%</span>
									</var>
								</span>
								<p>预期年化</p>
						...
						<div class="clearfix">
							...
							<div class="bottomRight fr">
								<span class="schedule" v-if="qxType == 99" style="text-align: left;">剩余&nbsp;{{leftCopies / 1000000 }}万元 </span>
								<span class="schedule bianyi" v-if="qxType != 99">剩余&nbsp;{{ leftCopies / 1000000 }}万元 </span>
							</div>
						</div>
					</div>
				...
				<li v-for="(list,index) in lists" @click="listFun(list)" v-if="lists">
					<div class="product-content">
						...
						<div class="clearfix">
							<div class="middleLeft fl">
								<span>{{ list.baseEarnings | fixNum }}
									<span   v-if="list.isJiangLi  != 1 ">%</span>
									<var v-if="list.isJiangLi == 1">+{{ parseFloat(list.jiangLiEarnings) | fixNum(1) }}
										<span>%</span>
									</var>
								</span>
								<p>预期年化</p>
						...
						<div class="clearfix clearfix2">
							...
							<div class="bottomRight fr">
								<span class="schedule" v-if="list.qxType == 99" style="text-align: left;">剩余&nbsp;{{list.leftCopies / 1000000}}万元 </span>
								<span class="schedule bianyi" v-if="list.qxType != 99">剩余&nbsp;{{list.leftCopies / 1000000}}万元 </span>
							</div>
						</div>
					</div>
				</li>
